...
def get_email_auth_code(email_user, email_pwd, max_retries=10, client_id=None, refresh_token=None, email_type='self', keyword=None):
    """
    ...
    if email_type == 'microsoft' and client_id and refresh_token:
        ...
        try:
            ...
            for attempt in range(max_retries):
                ...
                # 1. 先查询收件箱
                print("[*] 查询收件箱...")
                mail_content = youx.get_latest_mail(
                    email=email_user,
                    client_id=client_id,
...
class HuobiClient:
    ...
    def __init__(self, device_config, proxies=None):
        ...
        # 设备标识（确保一致性）
        self.vtoken = self.device.get(
            'android_id') or self.device.get('fingerprint', '')
        ...
        self.hb_uc_ua = 'c5a9aaf679120f736b34ffd69935ed7c'
    ...
    def bind_ga_process(self, email=None, password=None, client_id=None, refresh_token=None, email_type='self'):
        """
        执行GA绑定流程 (5步接口)
...
def save_account_json(email, password, uid, uc_token, register_time, device_config, client_id=None, refresh_token=None, invite_code=None, hb_pro_token=None, hb_uc_token=None, ga_key=None):
    """保存账号数据为JSON格式"""
    import os
...
def restore_client_from_account_data(account_data, proxies=None):
    ...
    try:
        ...
        # 恢复设备标识
        client.vtoken = device_config.get(
...
def register_worker_with_stop(email_data, stop_event, args, enable_sign_in=True):
    """带停止检查的注册工作线程"""
    ...
    while retry_count < MAX_RETRY_PER_ACCOUNT:
        ...
        if stop_event.is_set():
            print(f"[*] 收到停止信号，将完成当前账号处理后停止: {email}")
        ...
        try:
            ...
            if res.get("success"):
                ...
                if enable_sign_in:
                    ...
                    # 根据账号格式决定是否添加 client_id 和 refresh_token
                    if client_id and refresh_token:
                        # 新格式: 包含 client_id 和 refresh_token
                        account_line2 = f"{email}|{password}|{client_id}|{refresh_token}|{邀请码}|{ga_key}"
                        account_line = f"{email}|{password}|{client_id}|{refresh_token}|{uid}|{uc_token}|{register_time}|{device_config_json}|{ga_key}"
                    else:
                        # 旧格式: 只包含账号和密码
                        account_line2 = f"{email}|{password}|{邀请码}|{ga_key}"
                        account_line = f"{email}|{password}|{uid}|{uc_token}|{register_time}|{device_config_json}|{ga_key}"
...
def main():
    ...
    # 选择执行器类型：进程池避免 curl_cffi 多线程 TLS 错误
    use_process_pool = True  # 默认使用进程池

    if use_process_pool:
        print(f"[*] 使用 ProcessPoolExecutor (进程池) 避免 curl_cffi 多线程 TLS 错误")
